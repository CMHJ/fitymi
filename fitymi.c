...
main(int argc, char const *argv[])
{
    ...
    // Parse arguments
    double totalBuildTime = rand() % MAX_DEFAULT_BUILD_TIME + 1;
    if (argc == 2)
    {
    ...
    const uint8_t numTargets = rand() % maxTargets + 1;
    ...
    // printf("Total time taken to completion: %f\n", (getCurrentTimeStampMilliseconds() - buildStartTime)); // DEBUG
    ...
    int maxSourceFiles)
{
    ...
    for (int i = 1; i < numSourceFiles; i++)
    {
        ...
        currentPartBuildTime += partBuildTimes[i];
        usleep(currentPartBuildTime - getCurrentTimeStampMilliseconds());
    }
